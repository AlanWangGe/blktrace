...
static void mencode_movie(char *movie_dir)
{
	...
	snprintf(line, line_len, "ffmpeg -r 10 -y -i %s/%%10d-%s.svg.png -b:v 12M "
...
static void plot_io_movie(struct plot *plot)
{
	...
	list_for_each_entry(tf, &all_traces, list) {
		...
		while (i < cols) {
			...
			plot_latency(plot, tf->min_seconds, tf->max_seconds);

			/*plot->direction = PLOT_ACROSS;*/
			plot_queue_depth(plot, tf->min_seconds, tf->max_seconds);
			plot->direction = PLOT_ACROSS;
			plot_iops(plot, tf->min_seconds, tf->max_seconds);
